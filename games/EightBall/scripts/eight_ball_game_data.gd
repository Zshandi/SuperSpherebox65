...
func _post_draw_card_image(parent_control:Control, instance_date:GameInstanceData) -> void:
	instance_date.game_image_2 = Rand.pick(eight_ball_images)
	
	var num_sprites:int = 1
	if Rand.f() < multiple_chance:
		num_sprites = Rand.i_range(2, multiple_max)
	
	var scale := Vector2.ONE / sqrt(num_sprites)
	if Rand.f() < scale_chance:
		scale *= Rand.f_range(min_scale, max_scale)
	
	var rotate := Rand.f() < rotation_chance
	...
	for i in range(0, num_sprites):
		...
		if rotate:
			sprite.rotation = Rand.f_range(0, TAU)
